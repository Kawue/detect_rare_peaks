...
parser.add_argument("-t", "--threshold", dest='threshold', type=float, required=True, help="Picking treshold between 0 and 1.")
...
# pick files
mean_specs_picked_idxs = []
max_specs_picked_idxs = []
mean_specs_picked_mzs = []
max_specs_picked_mzs = []
for i,_ in enumerate(mean_specs):
    mean_spec_picked_idxs,_ = find_peaks(mean_specs[i], height=picking_t)
    max_spec_picked_idxs,_ = find_peaks(max_specs[i], height=picking_t)
    
    mean_specs_picked_idxs.append(list(mean_spec_picked_idxs))
    max_specs_picked_idxs.append(list(max_spec_picked_idxs))
    
    mzs = files[i].columns.astype(float)
    mean_specs_picked_mzs.append([mzs[idx] for idx in mean_spec_picked_idxs])
...
min_mz = np.amin(mzs)
max_mz = np.amax(mzs)
mean_mz_hist, mean_mz_bins = np.histogram([x for sublist in mean_specs_picked_mzs for x in sublist], bins=binning, range=(min_mz, max_mz))
max_mz_hist, max_mz_bins =np.histogram([x for sublist in max_specs_picked_mzs for x in sublist], bins=binning, range=(min_mz, max_mz))
...
rare_mzs_mean = []
for mz_list in mean_specs_picked_mzs:
    rare_mz = []
    for i in mz_list:
...
rare_mzs_max = []
for mz_list in max_specs_picked_mzs:
    rare_mz = []
    for i in mz_list:
